...
for i in range(1, len(sys.argv)):
    ...
    #plublication version below
    """
    pair_id = file_name.split('_')[5]
    pre_type= file_name.split('_')[3] #3
...
ax.set_ylim([-0.1, 2.1])
#plt.bar(x_coordinates, y_vals, yerr=y_errors)
#plt.boxplot(post_first8_vals, showfliers=False, whis=[5, 95])
plt.boxplot(post_first8_vals, showfliers=False)
#plt.title("Supervised Multilabel Classification Accuracy By Representation With PCA \n Non-Baseline Kruskal Wallis "+pval_to_str(group_pval))
plt.title("Fit MSE of First 8 by Post-Type")
plt.xticks(x_coordinates, post_first8_labels)
plt.xlabel("Representation")
plt.ylabel("MSE")
#plt.yscale("log") #adjust y-scale to log scale for revisions
#plt.axhline(y=1)
#plt.ylim([0,1])
savetitle = save_dir+"MSE_of_Post_Fit_first8.svg"
f.set_size_inches((4.9, 1.97))
f.set_dpi(1200)
f.tight_layout()
...
ax.spines['right'].set_visible(False)
#plt.bar(x_coordinates, y_vals, yerr=y_errors)
#plt.boxplot(post_last4_vals, showfliers=False, whis=[5, 95])
plt.boxplot(post_first8_vals, showfliers=False)
#plt.title("Supervised Multilabel Classification Accuracy By Representation With PCA \n Non-Baseline Kruskal Wallis "+pval_to_str(group_pval))
