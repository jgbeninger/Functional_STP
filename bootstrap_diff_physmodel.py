...
for i in range(1, len(sys.argv)):
    ...
    print(i)
    print(file_name)
    pair_id = file_name.split(cut_on)[9] #6 for standard, 8 for two step, 9 for windows
    pair_id = int(pair_id.split(".p")[0])
    pre_type = file_name.split(cut_on)[7] #4 for standard, 6 for two step, 7 for windows
    pre_type = pre_type.split(cut_on)[0]
    post_type = file_name.split(cut_on)[8] #5 for standard, 7 for two step, 8 for windows
...
def cre_accuracy_bootstrap(phys_params, model_params, row_labels, pre_index =0, target_index =1, partition_size=10):
    phys_raw_data = phys_params.copy()
    ...
    #print(raw_data)
    phys_scaler.fit(phys_raw_data)
    phys_scaled_arr = phys_scaler.transform(phys_raw_data)
    phys_pca_arr = pca.fit_transform(phys_scaled_arr)
    phys_data = phys_pca_arr.tolist()

    model_raw_data = model_params.copy()
    ...
    #print("printing raw_data")
    #print(raw_data)
    model_scaler.fit(model_raw_data)
    model_scaled_arr = model_scaler.transform(model_raw_data)
    model_pca_arr = pca.fit_transform(model_scaled_arr)
    model_data = model_pca_arr.tolist()
    ...
    phys_pca_arr = np.delete(phys_pca_arr, [31], axis=0)
    model_pca_arr = np.delete(model_pca_arr, [31], axis=0)
    ...
    for i in range(0, 1000000): 
        ...
        num_partitions = int(len(phys_pca_arr)/partition_size)
        ...
        phys_X_shuffle, phys_y_shuffle, model_X_shuffle, model_y_shuffle = shuffle(phys_pca_arr, categories, model_pca_arr, categories) #without replacement but with subsampling
        #print("X_shuffle")
        #print(X_shuffle)
        #print("y_shuffle")
        #print(y_shuffle)
        ...
        for n in range(1, num_partitions+1):
            ...
            #train_data = [row[0:(len(row)-1)] for row in train_set]
            #train_targets = [row[len(row)-1] for row in train_set]
            phys_train_data = np.append(phys_X_shuffle[:(n-1)*partition_size, :], phys_X_shuffle[n*partition_size:, :], axis=0)
            phys_train_targets = np.append(phys_y_shuffle[:(n-1)*partition_size], phys_y_shuffle[n*partition_size:], axis=0)
            
            model_train_data = np.append(model_X_shuffle[:(n-1)*partition_size, :], model_X_shuffle[n*partition_size:, :], axis=0)
            model_train_targets = np.append(model_y_shuffle[:(n-1)*partition_size], model_y_shuffle[n*partition_size:], axis=0)
            #print("train_data:")
            #print(train_data)
            ...
            #test_data = [row[0:(len(row)-1)] for row in test_set]
            #test_targets = [row[len(row)-1] for row in test_set]
            phys_test_data = phys_X_shuffle[(n-1)*partition_size:n*partition_size, :]
            phys_test_targets = phys_y_shuffle[(n-1)*partition_size:n*partition_size]
            
            model_test_data = model_X_shuffle[(n-1)*partition_size:n*partition_size, :]
            model_test_targets = model_y_shuffle[(n-1)*partition_size:n*partition_size]
            #print("test_data:")
            #print(test_data)
            #print("test_targets")
            #print(test_targets)
            ...
            #clf = MLPClassifier(hidden_layer_sizes=(50,50,50))
            #multi_target_svc = MultiOutputClassifier(clf, n_jobs=-1)
            clf.fit(phys_train_data, phys_train_targets)
            phys_accuracy = clf.score(phys_test_data, phys_test_targets)
            #print("appending accuracy")
            #print(accuracy)
            phys_accuracies.append(phys_accuracy)
            #test_accuracies.append(accuracy)
            ...
            #clf = MLPClassifier(hidden_layer_sizes=(50,50,50))
            #multi_target_svc = MultiOutputClassifier(clf, n_jobs=-1)
            clf.fit(model_train_data, model_train_targets)
            model_accuracy = clf.score(model_test_data, model_test_targets)
            #print("appending accuracy")
            #print(accuracy)
            ...
            #test baseline accuracy on this partition
            dummy_clf = DummyClassifier(strategy='most_frequent')
            dummy_clf.fit(phys_train_data, phys_train_targets)
            baseline_accuracy = dummy_clf.score(phys_test_data, phys_test_targets)
            predictions = dummy_clf.predict(phys_test_data)
            #print("printing baseline predictions")
            #print(predictions)
            #baseline_accuracies.append(baseline_accuracy)
            cross_baseline_accuracies.append(baseline_accuracy)
            #print("printing baseline_accuracy")
            #print(baseline_accuracy)
        ...
        phys_accuracy = statistics.mean(phys_accuracies)
        model_accuracy = statistics.mean(model_accuracies)
        baseline = statistics.mean(cross_baseline_accuracies)
        difference = model_accuracy - phys_accuracy 
        
        phys_test_accuracies.append(phys_accuracy)
        model_test_accuracies.append(model_accuracy)
        baseline_accuracies.append(baseline)
        differences.append(difference)
        
            #except:
            #print("ValueError: This solver needs samples of at least 2 classes in the data, but the data contains only one class: 0")
    return (phys_test_accuracies, model_test_accuracies, baseline_accuracies, differences)
...
#Alternates
phys_multi_accuracies, model_multi_accuracies, phys_multi_baseline, physmodel_multi_differences = cre_accuracy_bootstrap(phys_data, model_data, row_labels) #target_index=1 is post type
...
model_baseline_mean = statistics.mean(phys_multi_baseline)
model_baseline_sem = stat.sem(np.array(phys_multi_baseline))
model_pval = wilcoxon(model_multi_accuracies, phys_multi_baseline).pvalue
...
y_vals  = [phys_multi_accuracies, model_multi_accuracies, phys_multi_baseline]
x_coordinates = [i for i in range(1, len(y_vals)+1)]
f =plt.figure()
ax = plt.subplot(111)
...
plt.ylabel("Mean Accuracy")
#plt.ylim([0,1])
save_name = "./Figures/supplemental/multi_label_post_bootstrap_1.3mM.svg"
...
#repeat as bar plot
x_labels = ["phys","model", "baseline"]
y_vals  = [statistics.mean(phys_multi_accuracies), statistics.mean(model_multi_accuracies), statistics.mean(phys_multi_baseline)]
x_coordinates = [i for i in range(1, len(y_vals)+1)]
f =plt.figure()
ax = plt.subplot(111)
ax.spines['top'].set_visible(False)
...
plt.ylabel("Mean Accuracy")
#plt.ylim([0,1])
save_name = "./Figures/supplemental/multi_label_post_bootstrap_bar_1.3mM.svg"
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
frac_positive = len([1 for entry in physmodel_multi_differences if entry>0])/len(physmodel_multi_differences)
frac_nonneg = len([1 for entry in physmodel_multi_differences if entry>=0])/len(physmodel_multi_differences)
plt.hist(physmodel_multi_differences, bins=30)
plt.title("Post_type model vs phys histogram, positive frac: "+str(frac_positive )+" nonneg frac: "+str(frac_nonneg))
save_name = "./Figures/supplemental/multi_label_post_bootstrap_histmodelvsphys_1.3mM.svg"
#save_name = "./Figures/fig3/multi_label_post_bootstrap_bar_2mM.svg"
#save_name = "./Figures/fig3/multi_label_post_shuffle_bar_1.3mM.svg"
f.set_size_inches((2.3622, 2.465))
f.set_dpi(1200)
...
save_name = "./Figures/fig3/multi_label_post_bootstrap_histphys_1.3mM.svg"
#save_name = "./Figures/fig3/multi_label_post_bootstrap_bar_2mM.svg"
#save_name = "./Figures/fig3/multi_label_post_shuffle_bar_1.3mM.svg"
f.set_size_inches((2.3622, 2.465))
f.set_dpi(1200)
...
#Alternates
phys_multi_accuracies, model_multi_accuracies, phys_multi_baseline, physmodel_multi_differences = cre_accuracy_bootstrap(phys_data, model_data, row_labels, target_index=0) #target_index=1 is post type
#phys_multi_accuracies, phys_multi_baseline, phys_multi_differences = cre_accuracy_bootstrap_single_split(phys_data, row_labels)
phys_multi_mean = statistics.mean(phys_multi_accuracies)
phys_multi_sem = stat.sem(np.array(phys_multi_accuracies))
...
plt.ylabel("Mean Accuracy")
#plt.ylim([0,1])
save_name = "./Figures/supplemental/multi_label_pre_bootstrap_1.3mM.svg"
...
plt.ylabel("Mean Accuracy")
#plt.ylim([0,1])
save_name = "./Figures/supplemental/multi_label_pre_bootstrap_bar_1.3mM.svg"
...
frac_nonneg = len([1 for entry in physmodel_multi_differences if entry>=0])/len(physmodel_multi_differences)
plt.hist(physmodel_multi_differences, bins=30)
plt.title("Pre_type model vs phys histogram, positive frac: "+str(frac_positive )+" nonneg frac: "+str(frac_nonneg))

save_name = "./Figures/supplemental/multi_label_pre_bootstrap_histmodelvsphys_1.3mM.svg"
#save_name = "./Figures/fig3/multi_label_post_bootstrap_bar_2mM.svg"
#save_name = "./Figures/fig3/multi_label_post_shuffle_bar_1.3mM.svg"
f.set_size_inches((2.3622, 2.465))
f.set_dpi(1200)
...
save_name = "./Figures/fig3/multi_label_pre_bootstrap_histphys_1.3mM.svg"
#save_name = "./Figures/fig3/multi_label_post_bootstrap_bar_2mM.svg"
#save_name = "./Figures/fig3/multi_label_post_shuffle_bar_1.3mM.svg"
f.set_size_inches((2.3622, 2.465))
f.set_dpi(1200)
...
f.tight_layout()
plt.legend()
save_name = "./Figures/supplemental/stability_1.3mM.svg"
