...
brown='xkcd:burnt orange' 
pink='xkcd:hot pink'
#save folder
#out_folder = "/home/john/Pictures/STP_subtypes_paper/fig2/Kernel_plots/"
out_folder = "./Figures/fig2/"
...
for i in range(1, len(sys.argv)):
    ...
    #print(i)
    #print(file_name)
    pair_id = file_name.split(cut_on)[8] #6 for standard
    pair_id = int(pair_id.split(".p")[0])
    pre_type = file_name.split(cut_on)[6] #4 for standard
    pre_type = pre_type.split(cut_on)[0]
    post_type = file_name.split(cut_on)[7] #5 for standard
...
def data_centers(input_data, row_labels, target_index, id_index=2):
    ...
    return (np.asarray(group_medians), new_row_labels, unique_types, group_rows,group_ids)
...
for i in range(0, len(data_centers_pre)):
    ...
    if len(group_rows_pre[i]) > 3: #select only groups with at least 4 samples
        ...
        save_name = out_folder+"Pre_"+str(unique_pre_types[i])+".svg"
        ...
        ax.plot(kernel_x, kernel_y, label=label, color=colour)
        """
        if unique_pre_types[i] == 'pvalb':
            ax.plot(kernel_x, kernel_y, label=label, color=orange)
        elif unique_pre_types[i] == 'nr5a1':
            ax.plot(kernel_x, kernel_y, label=label, color=blue)
        elif unique_pre_types[i] == 'sim1':
            ax.plot(kernel_x, kernel_y, label=label, color=green)
        elif unique_pre_types[i] == 'sst':
            ax.plot(kernel_x, kernel_y, label=label, color=red)
        elif unique_pre_types[i] == 'vip':
            ax.plot(kernel_x, kernel_y, label=label, color=orange)
        elif unique_pre_types[i] == 'ntsr1':
            ax.plot(kernel_x, kernel_y, label=label, color=brown)
        elif unique_pre_types[i] == 'tlx3':
            ax.plot(kernel_x, kernel_y, label=label, color=pink)
        """
        
        #ax.plot(kernel_x, kernel_y, label=label, color=red) #colour changed from Black
        ...
        """
        plt.legend()
        #f.suptitle(str(unique_pre_types[i]) + " Pre Type")
        f.suptitle(str(cre_capital[unique_pre_types[i]]))
        f.set_size_inches((2.92, 2.06))
        f.set_dpi(600)
        f.tight_layout()
        plt.savefig(save_name, transparent=True)
        
        f.set_size_inches((5.84, 4.1))
        save_name = out_folder+"Big_Pre_"+str(unique_pre_types[i])+".svg"
        plt.savefig(save_name, transparent=True)
        #plt.show()
...
for i in range(0, len(data_centers_post)):
    ...
    if len(group_rows_post[i]) > 3: #select only groups with at least 4 samples
        f = plt.figure(tight_layout=True)
        ax = plt.subplot(111)
        save_name = out_folder+"Post_"+str(unique_post_types[i])+".svg"
        ...
        """
        if unique_post_types[i] == 'pvalb':
            ax.plot(kernel_x, kernel_y, label=label, color=orange)
        elif unique_post_types[i] == 'nr5a1':
            ax.plot(kernel_x, kernel_y, label=label, color=blue)
        elif unique_post_types[i] == 'sim1':
            ax.plot(kernel_x, kernel_y, label=label, color=green)
        elif unique_post_types[i] == 'sst':
            ax.plot(kernel_x, kernel_y, label=label, color=red)
        elif unique_post_types[i] == 'vip':
            ax.plot(kernel_x, kernel_y, label=label, color=orange)
        """
        ...
        """
        plt.legend()
        #f.suptitle(str(unique_post_types[i]) + " Post Type")
        f.suptitle(str(cre_capital[unique_post_types[i]]))
        f.set_size_inches((2.92, 2.06))
        f.set_dpi(600)
        f.tight_layout()
        plt.savefig(save_name, transparent=True)
        ...
        f.set_size_inches((5.84, 4.1))
        save_name = out_folder+"Big_Post_"+str(unique_post_types[i]+".svg")
        plt.savefig(save_name, transparent=True)
        #plt.show()
...
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.legend()f.set_size_inches((2.92, 2.06))
plt.show()       
