...
#transciptomic colour mapping
"""
trancriptomic_colours = {
    'pvalb':orange,
    'nr5a1':blue,
    'sim1':green,
    'sst':red,
...
def surrogate_centroids(input_data, row_labels, cluster_alg, id_index=2):
    ...
    cluster_sizes = [0 for i in range(0, num_clusters)]
    for i in range(0, num_clusters):
        ...
        #cluster_means[i] = [np.median(cluster_rows[i][:, j]) for j in range(0, len(input_data[0]))]
        ...
        cluster_sizes[i] = len(cluster_rows[i][:,0])
    return (np.asarray(cluster_medians), np.asarray(cluster_SDs), cluster_sizes, labels)
...
def data_centers(input_data, row_labels, target_index, id_index=2):
    ...
    group_medians = [[] for i in range(0, num_unique)]
    for i in range(0, num_unique):
        group_rows[i] = np.asarray(group_rows[i])
        #print("printing cluster "+str(i))
        #print(cluster_rows[i])
        #the below should return a 1d list of feature means for each cluster 
        group_medians[i] = [np.median(group_rows[i][:, j]) for j in range(0, len(input_data[0]))]
    return (np.asarray(group_medians), row_labels, unique_types, group_rows)
...
#uncomment to get km for phys
scaler = StandardScaler()
scaler.fit(model_data)
scaled_arr = scaler.transform(model_data)
plot_arr = pca.fit_transform(scaled_arr)

km_test = km.fit(plot_arr)
#cluster_centers_indices = km.cluster_centers_indices_
...
for i in range(0, 10):
    ...
    print(str(num_partitions)+" partitions")
    for n in range(0, num_partitions):
        #print(len(partitioned_list))
        #print(str(num_partitions)+" partitions")
        #print("n="+str(n))
...
with open('acc_pickles/OPTICS_acc_cross_vals_model.p', 'wb') as fp:
    pickle.dump(cluster_num_accuracy_dists, fp)
...
def test_cre_clusters(labeled_arr, cre_types, cre_entry_index, num_clusters, cre_title, save_name):
    """
    ...
    width = 1/(num_clusters+2)  # the width of the bars

    fig, ax = plt.subplots()
    for j in range(0, len(filtered_cre_types)):
        for i in range(0, num_clusters):
            value = cre_dist[filtered_cre_types[j]][i]
            rects1 = ax.bar(j -(width*num_clusters/2) + (i+0.5)*width, value, width, label=str(i), color=colour_set[i])
    
    
    
    x = np.arange(len(filtered_cre_types)) 
    ax.set_xticks(x)
    ax.set_xticklabels(filtered_cre_types)
    
    #plot 
    #plt.bar(x_coordinates, cluster_num_accuracies, yerr=cluster_num_sems)
    #title  = "Number by cluster of cre "+cre_title + " for "+str(num_clusters)+" clusters" 
    title  = cre_title+" by Cluster"
    plt.title(title)
    plt.ylabel("Num pairs")
    #legend_without_duplicate_labels(ax)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    fig.set_size_inches((4.6, 2.198))
    fig.set_dpi(1200)
    fig.tight_layout()
...
def cluster_cre_comp(labeled_arr, cre_types, cre_entry_index, num_clusters, cre_title, save_name):
    """
    labeled_arr: array with cluster labels as per merged_arr below
    ...
    num_bars = 0
    #create 2d array of cre_type count by cluster
    cre_by_cluster = [{} for i in range(0, num_clusters)]
    for i in range(0, len(labeled_arr)):
        cluster_label = labeled_arr[i][cluster_label_index]
        cre_label = labeled_arr[i][cre_entry_index]
        if cre_label in cre_by_cluster[cluster_label]:
            cre_by_cluster[cluster_label][cre_label] += 1
        else:
            cre_by_cluster[cluster_label][cre_label] = 1
            num_bars += 1
    
    width = 1/(len(cre_types)+2)  # the width of the bars

    fig, ax = plt.subplots()
    for j in range(0, len(cre_by_cluster)):
        print("Cluster "+str(j))
        i=0
        for key, entry in cre_by_cluster[j].items():
            value = entry
            print("cre_label: "+str(cre_capital[key]))
            print("cre_height: "+str(value))
            #rects1 = ax.bar(j -(width*num_clusters/2) + (i+0.5)*width, value, width, label=str(i), color=transcriptomic_colours[key])
            #use to generate legend of transcriptomic types
            rects1 = ax.bar(j -(width*num_clusters/2) + (i+0.5)*width, value, width, label=cre_capital[key], color=transcriptomic_colours[key])
            i+=1
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([i for i in range(0, num_clusters)])
...
def save_single_kernel(mu_amps, mu_baseline, title_name, file_name, colour="xkcd:blue"):
    ...
    plt.plot(kernel_x, kernel_y, label=label, color=colour) #match colour to bar plots
...
pre_index = 1
post_index = 2
save_name_pre = "./fig2/cluster_comp_pre.svg"
save_name_post = "./fig2/cluster_comp_post.svg"
#exclusions in tes-cre_cluters to remove gabaergic pretypes as needed
test_cre_clusters(merged_arr, unique_pre, pre_index, num_clusters, "Pre-Types", save_name_pre)
test_cre_clusters(merged_arr, unique_post, post_index, num_clusters, "Post-Types", save_name_post)

save_name_inverted_pre="./fig2/cluster_comp_inverted_pre.svg"
save_name_inverted_post="./fig2/cluster_comp_inverted_post.svg"
...
#plot single kernels
single_kernel_dir = "./single_kernels/"
for i in range(0, len(data_centers_post)):
    label = "cluster "+str(i)+": "+str(syn_by_cluster_surr[i])+" synapses, "+str(runs_by_cluster_surr[i]) +" runs"
    mu_amps = [data_centers_post[i,1], data_centers_post[i, 2], data_centers_post[i,3]] #add 1 to all values
    mu_baseline = data_centers_post[i,0] 
    ...
    file_name = single_kernel_dir + "rodent_post_raw_mean"+unique_post_types[i]+".svg"
    title_name = cre_capital[unique_post_types[i]]+ " post"
    colour = transcriptomic_colours[unique_post_types[i]]
    save_single_kernel(mu_amps, mu_baseline, title_name, file_name, colour=colour)
...
for i in range(0, len(data_centers_pre)):
    ...
    #print(mu_amps)
    #print("mu_baseline: "+str(mu_baseline))
    file_name = single_kernel_dir + "rodent_pre_raw_mean"+unique_pre_types[i]+".svg"
    title_name = cre_capital[unique_pre_types[i]]+ " pre"
    colour = transcriptomic_colours[unique_pre_types[i]]
    save_single_kernel(mu_amps, mu_baseline, title_name, file_name, colour=colour)
...
def supervised_means(train_set, train_target, unique_post, num_unique_post):
    ...
    prediction_rows = [[] for i in range(0, num_unique_post)]
    #convolved_rows = [[] for i in range(0, len(train_set[0,:]))]
    convolved_rows = [[] for i in range(0, num_unique_post)]
    #create list of lists corresponding to all synapse fit rows by cluster
    ...
    for i in range(0, len(clf_predictions)):
        ...
        for j in range(0, len(unique_post)):
            if clf_predictions[i] == unique_post[j]:
                #print("j="+str(j))
                #print("len(prediction_rows)="+str(len(prediction_rows)))
                #print("model_data[i]")
                #print(model_data[i])
                prediction_rows[j].append(model_data[i][:])
                params = model_data[i][:]
                
                mu_amps = [model_data[i][1], model_data[i][2], model_data[i][3]] #add 1 to all values
                mu_baseline = model_data[i][0] 
                kernel_x, kernel_y = gen_kernel(mu_amps, mu_kernel_taus, 1000, mu_baseline=mu_baseline, dt=1)
                convolved_rows[j].append(kernel_y)
                #print("printing kernel_y")
                #print(kernel_y)
    
    prediction_medians = [[0 for j in range(0, len(model_data[0]))] for i in range(0, num_unique_post)]
    convolved_medians = [[0 for j in range(0, len(model_data[0]))] for i in range(0, num_unique_post)]
    param_SDs = [[0 for j in range(0, len(model_data[0]))] for i in range(0, num_unique_post)]
    for i in range(0, num_unique_post):
        if len(prediction_rows[i]) > 1:
            ...
            #print(prediction_rows[i])
            #the below should return a 1d list of feature means for each cluster 
            prediction_medians[i] = [np.median(prediction_rows[i][:, j]) for j in range(0, len(model_data[0]))]
            param_SDs[i] = [np.std(prediction_rows[i][:, j]) for j in range(0, len(model_data[0]))]
            #this is where I would add a kernel conversion and temporal mean using prediction_rows
        if len(convolved_rows[i]) > 1:
            convolved_rows[i] = np.asarray(convolved_rows[i])
            #calc mean at each timestep
            #print("len(convolved_rows[i][:]) = "+str(len(convolved_rows[i])))
            #print(convolved_rows[i])
            convolved_medians[i] = [np.median(convolved_rows[i][:, j]) for j in range(0, len(convolved_rows[i][0]))]
            #print("printing convolved_means[i]")
            #print(convolved_means[i])
    #print("printing cluster_means")
    #print(prediction_means)
    return (np.asarray(prediction_rows), np.asarray(prediction_medians), param_SDs, convolved_medians)

prediction_rows_pre, clf_means_pre, clf_SDs_pre, temporal_means_pre = supervised_means(train_set, train_target_pre, unique_pre, num_unique_pre)
prediction_rows_post, clf_means_post, clf_SDs_post, temporal_means_post = supervised_means(train_set, train_target_post, unique_post, num_unique_post)
...
#all data for means and SDs by param:
...
for i in range(0, len(clf_means_pre)):
    if len(prediction_rows_pre[i]) > 1:
        ...
        kernel_x, kernel_y = gen_kernel(mu_amps, mu_kernel_taus, 1000, mu_baseline=mu_baseline, dt=1)
        #kernels_over_time.append(kernel)
        #use temporal means instead of mean of params
        temporal_means = temporal_means_pre[i]
        #print(temporal_means)
        plt.plot(kernel_x, temporal_means, label=unique_pre[i])
        #plt.plot(kernel_x, kernel_y, label=unique_pre[i]) #mean of params plot
        #plt.plot(kernel_x, kernel_y)
    else:
        print(unique_pre[i])
plt.legend()
...
for i in range(0, len(clf_means_pre)):
    ...
    #print(mu_amps)
    #print("mu_baseline: "+str(mu_baseline))
    file_name = single_kernel_dir + "rodent_pre_SVM"+unique_pre_types[i]+".svg"
    title_name = cre_capital[unique_pre_types[i]]+ " pre"
    colour = transcriptomic_colours[unique_pre_types[i]]
    save_single_kernel(mu_amps, mu_baseline, title_name, file_name, colour=colour)
...
for i in range(0, len(clf_means_post)):
    if len(prediction_rows_post[i]) > 1:
        ...
        kernel_x, kernel_y = gen_kernel(mu_amps, mu_kernel_taus, 1000, mu_baseline=mu_baseline, dt=1)
        #kernels_over_time.append(kernel)
        temporal_means = temporal_means_post[i]
        #print(temporal_means)
        plt.plot(kernel_x, temporal_means, label=unique_post[i])
...
for i in range(0, len(clf_means_post)):
    ...
    #print(mu_amps)
    #print("mu_baseline: "+str(mu_baseline))
    file_name = single_kernel_dir + "rodent_post_SVM"+unique_post_types[i]+".svg"
    title_name = cre_capital[unique_post_types[i]]+ " post"
    colour = transcriptomic_colours[unique_post_types[i]]
    save_single_kernel(mu_amps, mu_baseline, title_name, file_name, colour=colour)
...
f.set_dpi(1200)
f.tight_layout()
save_name = "./fig2/Unsupervised_kernel_plot.svg"
plt.savefig(save_name, transparent=True)
#plt.show()  


#plot bar plot of SD means
f = plt.figure()
ax = plt.subplot(111)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
#plt.title("Kernel of SRP OPTICS mean values with baseline") "Kernels by Cluster Human"
plt.title("Model SD")
model_SDs = surrogate_means[:, 4]
model_SD_err = surrogate_SDs[:, 4] #note this is the standard deviation of the standard deviation
for i in range(0, len(cluster_sizes)):
    model_SD_err[i] = model_SD_err[i]/math.pow(cluster_sizes[i], 1/2) #convert to SEM
x = [i for i in range(0, len(model_SDs))]
plt.bar(x, model_SDs, yerr=model_SD_err, color=colour_set[0:5])
ax.set_xticks(x)
ax.set_xticklabels(['0', '1', '2', '3', '4'])
f.set_size_inches((1.921, 3.59))
f.set_dpi(1200)
plt.xlabel("Cluster")
plt.ylabel("Model SD")
f.tight_layout()
save_name = "./fig2/SD_by_Cluster.svg"
plt.savefig(save_name, transparent=True)
...
for i in range(0, len(surrogate_means)):
    ...
    #print(mu_amps)
    #print("mu_baseline: "+str(mu_baseline))
    file_name = single_kernel_dir + "rodent_cluster_"+str(i)+".svg"
    title_name = "R"+str(i)
...
plt.xlabel("PC1")
plt.ylabel("PC2")
f.set_size_inches((3.47, 3.29))
f.set_dpi(1200)
f.tight_layout()
plt.legend()
save_name = "./fig2/Model_pre_prediction_scatter.svg"
plt.savefig(save_name)
#plt.show()

#----------------------------------------------------------------

#repeat but for comparison plots
#scatter plots by predicted type
prediction_rows_pre_sct, clf_means_pre_sct = supervised_means_scatter(train_set, train_target_pre, unique_pre, num_unique_pre)
prediction_rows_post_sct, clf_means_post_sct = supervised_means_scatter(train_set, train_target_post, unique_post, num_unique_post)
...
save_name = "./fig2/Model_pre_prediction_scatter_comparison.svg"
plt.savefig(save_name)
#plt.show()
#----------------------------------------------------------------
...
save_name = "./fig2/Model_post_prediction_scatter.svg"
plt.savefig(save_name)
#plt.show()

#----------------------------------------------------------------
#repeat for comparison plot
...
ax.get_xaxis().set_visible(False)
ax.get_yaxis().set_visible(False)
save_name = "./fig2/Model_post_prediction_scatter_comparison.svg"
plt.savefig(save_name)
#plt.show()
...
scaled_arr = scaler.transform(model_data)
pca_result = pca.fit_transform(scaled_arr)
cluster_predictions = unsupervised_scatter(pca_result) train_set
"""
print("running unsupervised_scatter")
...
ax.get_xaxis().set_visible(False)
ax.get_yaxis().set_visible(False)
save_name = "./fig2/Model_cluster_scatter_for_comparison.svg"
...
f.tight_layout()
#plt.legend(bbox_to_anchor=(1.0, 1.0))
save_name = "./fig2/Model_cluster_scatter_PC34.svg"
plt.savefig(save_name, transparent=True)
#plt.show()
