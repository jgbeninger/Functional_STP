...
for i in range(1, len(sys.argv)):
    ...
    #print(i)
    print(file_name)
    pair_id = file_name.split(cut_on)[4] #6 for standard #8 for two phase, 9 for windows
    pair_id = int(pair_id.split(".p")[0])
    pre_type = file_name.split(cut_on)[2] #4 for standard 6 for two phase, 7 for windows
    pre_type = pre_type.split(cut_on)[0]
    pre_type = pre_type.split("/")[1]
    post_type = file_name.split(cut_on)[3] #5 for standard 7 for two phase, 8 for windows
...
pickle_file = open("Extracted_STP_1.3mM_Human.p", "rb")
...
for i in range(0, len(model_data)):
    if cluster_predictions[i] > -1:
        out_table[i].append('H'+str(cluster_predictions[i])) #cluster
    else:
        out_table[i].append('H5')
...
print(out_arr)
np.savetxt("fig3/1.3mMhumanparams.csv", out_arr, delimiter=",", fmt='%s')
...
out_arr = out_arr[ind] #sort by cluster id
print(out_arr)
np.savetxt("fig3/1.3mMhumanparams.csv", out_arr[:, 0:row_length-1], delimiter=",", fmt='%s')
"""
...
#plt.xlabel("PC1")
#plt.ylabel("PC2")
save_name = "./fig3/Human_scatter.svg"
...
plt.xlabel("PC3")
plt.ylabel("PC4")
save_name = "./fig3/Human_scatter_pc34.svg"
...
plt.xlabel("PC2")
plt.ylabel("PC3")
save_name = "./fig3/Human_scatter_pc23.svg"
f.set_size_inches((3.76, 3.15))
f.set_dpi(1200)
f.tight_layout()
plt.savefig(save_name)
...
plt.xlabel("PC2")
plt.ylabel("PC4")
save_name = "./fig3/Human_scatter_pc24.svg"
f.set_size_inches((3.76, 3.15))
f.set_dpi(1200)
f.tight_layout()
plt.savefig(save_name)
...
plt.xlabel("PC1")
plt.ylabel("PC2")
save_name = "./fig3/Model_cluster_pre_scatter.svg"
f.set_dpi(600)
...
plt.xlabel("PC1")
plt.ylabel("PC2")
save_name = "./fig3/Model_cluster_post_scatter.svg"
f.set_dpi(600)
f.set_size_inches((2.56, 3.35))
#f.t1ght_layout()
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
