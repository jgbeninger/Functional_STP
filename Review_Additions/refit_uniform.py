...
for j in range(0, 1000):
    uniform_fits = gen_uniform_srp(_default_parameter_bounds_simple(), num_fits=86)
    ...
    silhouette_score = metrics.silhouette_score(pca_model_result_rodent, clustering.labels_)
    scatter_plot_clustering(unsupervised_scatter(clustering, pca_model_result_rodent), "./reviewfigssmall/scatters_prefit/scatter"+str(j), silhouette_score)
    """
    except:
    ...
    try:
        clustering = OPTICS(min_samples=best_min, metric="sqeuclidean").fit(pca_model_result_rodent) 
        silhouette_score = metrics.silhouette_score(pca_model_result_rodent, clustering.labels_)
        scatter_plot_clustering(unsupervised_scatter(clustering, pca_model_result_rodent), "./reviewfigssmall/scatters_postfit/scatter"+str(j), silhouette_score)
    except:
        print("no OPTICS clustering")
...
save_name = "./reviewfigssmall/uniformprefit_peak_silhouette_coefficients2.svg"
...
save_name = "./reviewfigssmall/uniformpostfit_peak_silhouette_coefficients2.svg"
#f.set_size_inches((2.3622, 2.465))
#f.set_dpi(1200)
f.tight_layout()
plt.savefig(save_name, Transparent=True)
...
save_name = "./reviewfigssmall/uniformprefit_cluster_num.svg"
#f.set_size_inches((2.3622, 2.465))
#f.set_dpi(1200)
f.tight_layout()
plt.savefig(save_name, Transparent=True)
...
save_name = "./reviewfigssmall/uniformpostfit_cluster_num.svg"
#f.set_size_inches((2.3622, 2.465))
#f.set_dpi(1200)
f.tight_layout()
plt.savefig(save_name, Transparent=True)
...
name = "./reviewfigssmall/refit_pickles/refitresults" +sys.argv[1] +".p"
file = open(name, "wb")
pickle.dump(results, file)

#pickle model fits
results = [srp_params_pre, srp_params_post]
name = "./reviewfigssmall/refit_pickles/model_params" +sys.argv[1] +".p"
file = open(name, "wb")
pickle.dump(results, file)