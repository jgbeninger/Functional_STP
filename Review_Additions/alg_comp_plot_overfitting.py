...
file_names_pre_phys = ["physprealgcompdists1.3mMfulltrainingn10000r1.p"
                       #"physprealgcompdists1.3mMfulltrainingn3000r2.p",
                       #"physprealgcompdists1.3mMfulltrainingn3000r3.p"
                       ]

file_names_pre_model = ["modelprealgcompdists1.3mMfulltrainingn10000r1.p"
                       #"modelprealgcompdists1.3mMfulltrainingn3000r2.p",
                       #"modelprealgcompdists1.3mMfulltrainingn3000r3.p"
                       ]

file_names_post_phys = [#"physpostalgcompdists1.3mMfulln100r1.p",
                        "physpostalgcompdists1.3mMfulltrainingn10000r1.p"
                        #"physpostalgcompdists1.3mMfulltrainingn3000r2.p",
                        #"physpostalgcompdists1.3mMfulltrainingn3000r3.p"
                        ]
file_names_post_model = [#"modelpostalgcompdists1.3mMfulln100r1.p",
                         "modelpostalgcompdists1.3mMfulltrainingn10000r1.p"
                         #"modelpostalgcompdists1.3mMfulltrainingn3000r2.p",
                         #"modelpostalgcompdists1.3mMfulltrainingn3000r3.p"
                         ]
...
for filename in file_names_pre_phys:
    file = 'supervised_dists_overfit_testing/'+ filename 
    infile = open(file,'rb')
    phys_pre_alg_dists, alg_labels = pickle.load(infile)
    infile.close()
    for i in range(0, len(alg_labels)):
        pre_phys_accs[i] = pre_phys_accs[i] + phys_pre_alg_dists[i][3]
        pre_phys_baselines[i] = pre_phys_baselines[i] + phys_pre_alg_dists[i][4]
...
for filename in file_names_pre_model:
    ...
    infile.close()
    for i in range(0, len(alg_labels)):
        pre_model_accs[i] = pre_model_accs[i] + model_pre_alg_dists[i][3]
        pre_model_baselines[i] = pre_model_baselines[i] + model_pre_alg_dists[i][4]
...
for filename in file_names_post_model:
    ...
    infile.close()
    for i in range(0, len(alg_labels)):
        post_model_accs[i] = post_model_accs[i] + model_post_alg_dists[i][3]
        post_model_baselines[i] = post_model_baselines[i] + model_post_alg_dists[i][4]
...
for filename in file_names_post_phys:
    ...
    infile.close()
    for i in range(0, len(alg_labels)):
        post_phys_accs[i] = post_phys_accs[i] + phys_post_alg_dists[i][3]
        post_phys_baselines[i] = post_phys_baselines[i] + phys_post_alg_dists[i][4]
...
#f.set_dpi(1200)
f.tight_layout()
save_name = "./supplemental_figs/alg_comp_post_model_overfitting.svg"
...
plt.boxplot(y_vals, showmeans=True, meanline=True, medianprops=medianprops)
#plt.title("Supervised Multilabel Classification Accuracy By Representation With PCA \n Non-Baseline Kruskal Wallis "+pval_to_str(group_pval))
plt.title("Postsynaptic Subclass Prediction By Algorithm Phys.svg")
plt.xticks(x_coordinates, alg_labels)
#plt.xlabel("Representation")
plt.ylabel("Accuracy")
#plt.ylim([0,1])
...
#f.set_dpi(1200)
f.tight_layout()
save_name = "./supplemental_figs/alg_comp_post_phys_overfitting"
plt.savefig(save_name, transparent=True)
plt.show()
...
#f.set_dpi(1200)
f.tight_layout()
save_name = "./supplemental_figs/alg_comp_pre_phys_overfitting.svg"
plt.savefig(save_name, transparent=True)
plt.show()
...
#f.set_dpi(1200)
f.tight_layout()
save_name = "./supplemental_figs/alg_comp_pre_model_overfitting.svg"
plt.savefig(save_name, transparent=True)
plt.show()
