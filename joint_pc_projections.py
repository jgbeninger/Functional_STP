...
for i in range(len(rodent_cluster_predictions)-1, -1, -1):
    ...
    plt.scatter(rodent_cluster_predictions[i][:, 0], rodent_cluster_predictions[i][:, 1], label=label, color=colour_set[i])
    """
    else:
...
for i in range(len(human_cluster_predictions)-1, -1, -1):
    ...
    label = ""+str(i)
    #if isinstance(prediction_rows_post_sct[i], np.ndarray):
    plt.scatter(human_cluster_predictions[i][:, 0], human_cluster_predictions[i][:, 1], color=human_colour_set[i], label=label)
    """
    else:
        print("exception on prediction i= "+str(i))
        #plt.plot(kernel_x, kernel_y)
