...
def cre_accuracy_bootstrap(params, row_labels, pre_index =0, target_index =1, partition_size=10):
    ...
    for i in range(0, 100000): 
        ...
        test_accuracies.append(statistics.mean(cross_accuracies))
        baseline_accuracies.append(statistics.mean(cross_baselines))
            #except:
            #print("ValueError: This solver needs samples of at least 2 classes in the data, but the data contains only one class: 0")
    return (test_accuracies, baseline_accuracies)
...
#Alternates
phys_multi_accuracies, phys_multi_baseline = cre_accuracy_bootstrap(phys_data, row_labels)
phys_multi_mean = statistics.mean(phys_multi_accuracies)
phys_multi_sem = stat.sem(np.array(phys_multi_accuracies))
...
model_multi_accuracies, model_multi_baseline = cre_accuracy_bootstrap(model_data, row_labels)
model_multi_mean = statistics.mean(model_multi_accuracies)
model_multi_sem = stat.sem(np.array(model_multi_accuracies))
...
phys_multi_accuracies, phys_multi_baseline = cre_accuracy_bootstrap(phys_data, row_labels, target_index =0)
phys_multi_mean = statistics.mean(phys_multi_accuracies)
phys_multi_sem = stat.sem(np.array(phys_multi_accuracies))
...
model_multi_accuracies, model_multi_baseline = cre_accuracy_bootstrap(model_data, row_labels, target_index =0)
model_multi_mean = statistics.mean(model_multi_accuracies)
model_multi_sem = stat.sem(np.array(model_multi_accuracies))
