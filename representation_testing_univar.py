...
for i in range(1, len(sys.argv)):
    ...
    print(i)
    print(file_name)
    pair_id = file_name.split(cut_on)[10] #6 for standard, 8 for two step, 9 for windows
    print("printing first cut")
    print(pair_id)
    pair_id = int(pair_id.split(".p")[0])
    pre_type = file_name.split(cut_on)[8] #4 for standard, 6 for two step, 7 for windows
    pre_type = pre_type.split(cut_on)[0]
    post_type = file_name.split(cut_on)[9] #5 for standard, 7 for two step, 8 for windows
...
plt.ylabel("Mean Accuracy")
#plt.ylim([0,1])
save_name = "./Figures/fig3/univar_multi_label_post_1.3mM.svg"
f.set_size_inches((3.35, 2.5))
f.set_dpi(1200)
f.tight_layout()
...
plt.ylabel("Mean Accuracy")
#plt.ylim([0,1])
save_name = "./Figures/fig3/univar_multi_label_pre_1.3mM.svg"
f.set_size_inches(((3.35, 2.5)))
f.set_dpi(1200)
f.tight_layout()
plt.savefig(save_name)
...
with open('acc_pickles_univar/km_acc_cross_vals_phys.p', 'wb') as fp:
...
f.tight_layout()
plt.legend()
save_name = "./Figures/fig4/univar_stability_1.3mM.svg"
plt.savefig(save_name)
plt.show()
