...
for i in range(1, len(sys.argv)):
    ...
    print(i)
    print(file_name)
    pair_id = file_name.split(cut_on)[8] #6 for standard, 8 for two step
    pair_id = int(pair_id.split(".p")[0])
    pre_type = file_name.split(cut_on)[6] #4 for standard, 6 for two step
    pre_type = pre_type.split(cut_on)[0]
    post_type = file_name.split(cut_on)[7] #5 for standard, 7 for two step
