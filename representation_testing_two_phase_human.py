...
for i in range(1, len(sys.argv)):
    ...
    try: #try testing PPR criterion
        ...
        continue #if no PPR is available skip synapse
    ...
    try:
        ...
        #adjust this so it does not add all missing rows if only some are missing
        print("missing entry skipped")
        continue #skip if measures missing
        new_row.append('missing_entry')
        new_row.append('missing_entry')
        new_row.append('missing_entry')
        new_row.append('missing_entry')
...
x_labels = ["phys_multi \n"+pval_to_str(phys_pval),"model_multi \n"+pval_to_str(model_pval), "model_baseline"]
...
plt.bar(x_coordinates, cluster_num_accuracies, yerr=cluster_num_sems)
plt.title("Cross Validation Accuracy of Different Cluster Numbers, PCA, SRP Model")
