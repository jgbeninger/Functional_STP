...
def cre_accuracy_bootstrap(params, row_labels, pre_index =0, target_index =1, partition_size=10):
    ...
    alg_outputs = [[-1 for i in range(0, 6)] for j in range(0, len(alg_labels))]
    ...
    for i in range(0, 10000): 
        ...
        cross_train_accuracies = [[] for i in range(0, len(alg_labels))]
        cross_train_baselines = [[] for i in range(0, len(alg_labels))]
        ...
        for n in range(1, num_partitions+1):
            ...
            for index in range(0, len(clf_list)):
                ...
                clf.fit(train_data, train_targets)
                accuracy = clf.score(test_data, test_targets)
                train_accuracy = clf.score(train_data, train_targets)
                print("appending accuracy")
                print(accuracy)
                ...
                dummy_clf.fit(train_data, train_targets)
                baseline_accuracy = dummy_clf.score(test_data, test_targets)
                training_baseline_accuracy = dummy_clf.score(train_data, train_targets)
                predictions = dummy_clf.predict(test_data)
                ...
                cross_train_accuracies[index].append(train_accuracy)
                #test_accuracies.append(accuracy)
                ...
                cross_train_baselines[index].append(training_baseline_accuracy)
                #print("printing baseline_accuracy")
                #print(baseline_accuracy)
        ...
        for i in range(0, len(alg_labels)):
            ...
            train_accuracy = statistics.mean(cross_train_accuracies[i])
            train_baseline = statistics.mean(cross_train_baselines[i])
            train_difference = train_accuracy - train_baseline
            ...
            if alg_outputs[index][0] == -1: #if not filled
                ...
                alg_outputs[i][3] = [train_accuracy]
                alg_outputs[i][4] = [train_baseline]
                alg_outputs[i][5] = [train_difference]
                ...
                alg_outputs[i][3].append(train_accuracy)
                alg_outputs[i][4].append(train_baseline)
                alg_outputs[i][5].append(train_difference)
...
phys_alg_outputs, phys_alg_labels =cre_accuracy_bootstrap(phys_data, row_labels, target_index =0)
model_alg_outputs, phys_alg_labels =cre_accuracy_bootstrap(model_data, row_labels, target_index =0)

#pickle the outputs

with open('./supervised_dists/physprealgcompdists1.3mMfulltrainingn10000r1.p', 'wb') as handle:
    pickle.dump((phys_alg_outputs, phys_alg_labels), handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('./supervised_dists/modelprealgcompdists1.3mMfulltrainingn10000r1.p', 'wb') as handle:
