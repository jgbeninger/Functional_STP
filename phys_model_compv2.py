...
#1.3mM
with open("./Figures/dists/modelpostbootstrapdists1.3mMfullmlp.p", "rb") as input_file:
    model_post_1_3mM = pickle.load(input_file)
    
   
with open("./Figures/dists/modelprebootstrapdists1.3mMfullmlp.p", "rb") as input_file:
    model_pre_1_3mM = pickle.load(input_file)

    
with open("./Figures/dists/physpostbootstrapdists1.3mMfullmlp.p", "rb") as input_file:
    phys_post_1_3mM = pickle.load(input_file)
  

with open("./Figures/dists/physprebootstrapdists1.3mMfullmlp.p", "rb") as input_file:
...
#2mM
"""
with open("./Figures/dists/modelprebootstrapdists2mMfullmlp.p", "rb") as input_file:
...
with open("./Figures/dists/physprebootstrapdists2mMfullmlp.p", "rb") as input_file:
...
#calc diffs
combined_pre_diffs  = np.asarray(model_pre_1_3_differences) - np.asarray(phys_pre_1_3_differences)
combined_post_diffs  =  np.asarray(model_post_1_3_differences) - np.asarray(phys_post_1_3_differences)
...
#print accuraciesstats
#pre model
print("pre model accuracy="+str(np.mean(model_pre_1_3_accuracies)))
print("pre model baseline="+str(np.mean(model_pre_1_3_baseline)))
print("pre model sem="+str(stats.sem(model_pre_1_3_accuracies)))

#pre phys
print("pre phys accuracy="+str(np.mean(phys_pre_1_3_accuracies)))
print("pre phys baseline="+str(np.mean(phys_pre_1_3_baseline)))
print("pre phys sem="+str(stats.sem(phys_pre_1_3_accuracies)))


#post model
print("post model accuracy="+str(np.mean(model_post_1_3_accuracies)))
print("post model baseline="+str(np.mean(model_post_1_3_baseline)))
print("post model sem="+str(stats.sem(model_post_1_3_accuracies)))


#post phys
print("post phys accuracy="+str(np.mean(phys_pre_1_3_accuracies)))
print("post phys baseline="+str(np.mean(phys_post_1_3_baseline)))
print("post phys sem="+str(stats.sem(phys_pre_1_3_accuracies)))
...
frac_positive = len([1 for entry in combined_pre_diffs if entry>0])/len(combined_pre_diffs)
frac_nonneg = len([1 for entry in combined_pre_diffs if entry>=0])/len(combined_pre_diffs)
plt.hist(combined_pre_diffs, bins=30)
plt.title("Pre_type model 1.3mM model minus phys, positive frac: "+str(frac_positive )+" nonneg frac: "+str(frac_nonneg))

save_name = "./Figures/Supplementals/multi_label_pre_bootstrap_histmodevsphysmlp.svg"
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
frac_positive = len([1 for entry in combined_post_diffs if entry>0])/len(combined_post_diffs)
frac_nonneg = len([1 for entry in combined_post_diffs if entry>=0])/len(combined_post_diffs)
plt.hist(combined_pre_diffs, bins=30)
plt.title("post_type model 1.3mM model minus phys, positive frac: "+str(frac_positive )+" nonneg frac: "+str(frac_nonneg))

save_name = "./Figures/Supplementals/multi_label_post_bootstrap_histmodevsphysmlp.svg"
#save_name = "./Figures/fig3/multi_label_post_bootstrap_bar_2mM.svg"
#save_name = "./Figures/fig3/multi_label_post_shuffle_bar_1.3mM.svg"
f.set_size_inches((2.3622, 2.465))
f.set_dpi(1200)
